...
func TestAddZipFile(t *testing.T) {
	...
	err := f.AddZipFile("test_data/test.zip")
...
func TestAddZipFileConcatenatesSpecialFiles(t *testing.T) {
	t.Run("Scala Akka reference.conf", func(t *testing.T) {
		...
		err := f.AddZipFile("test_data_3/z1.zip")
		r.NoError(err)
		err = f.AddZipFile("test_data_3/z2.zip")
		r.NoError(err)
		f.Close()

		actualContents, err := getZipContents("zip_files_with_reference_conf.zip")
		r.NoError(err)
		z1Contents, err := getZipContents("test_data_3/z1.zip")
		r.NoError(err)
		z2Contents, err := getZipContents("test_data_3/z2.zip")
		r.NoError(err)
...
func TestAddFiles(t *testing.T) {
	...
	f.Suffix = []string{"zip"}
	err := f.AddFiles("test_data")
	require.NoError(t, err)
	err = f.AddFiles("test_data_2")
	require.NoError(t, err)
	f.Close()
...
func TestStoreSuffix(t *testing.T) {
	...
	png := r.File[0]
	assert.Equal(t, "test_data/kitten.png", png.Name)
