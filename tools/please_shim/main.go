...
func findRootAndReadConfigFilesOnly() *core.Configuration {
	...
	if opts.BuildFlags.RepoRoot != "" {
		...
		if err := core.ReadDefaultGlobalConfigFilesOnly(fs.HostFS, config); err != nil {
	...
	if err := core.ReadDefaultConfigFilesOnly(fs.HostFS, config, opts.BuildFlags.Profile); err != nil {
