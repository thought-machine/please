...
int main(int argc, char* argv[]) {
    ...
    // Network namespace is **NOT** sandboxed by default but it can be enabled if `SHARE_NETWORK=0` env is set
    const char* share_network_env = getenv("SHARE_NETWORK");
    const bool unshare_network = share_network_env != NULL && !strcmp(share_network_env, "0");
