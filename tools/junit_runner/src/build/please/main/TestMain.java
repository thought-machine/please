...
public class TestMain {
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (Class<?> testClass : allClasses) {
      if (testClass.getAnnotation(NotATest.class) == null) {
    ...
    if (System.getenv("COVERAGE") == null) {
      ...
      for (Class<?> clz : testClasses) {
        TestSuiteResult result = tester.runTest(clz);
        Document doc = testReporter.writeSuiteResult(result);
        ...
        PrettyPrintingXmlWriter.writeXMLDocumentToFile(RESULTS_DIR + "/TEST-" + result.testClassName + ".xml", doc);
      }
    } else {
      // plz cover
      ...
      CoverageRunResult results = coverage.runTests(testClasses);

      Document coverageDoc = coverageReporter.writeResults(results.coverageBuilder, results.testClassNames);
      ...
      for (TestSuiteResult suiteResult : results.testResults) {
        Document doc = testReporter.writeSuiteResult(suiteResult);
        PrettyPrintingXmlWriter.writeXMLDocumentToFile(RESULTS_DIR + "/TEST-" + suiteResult.testClassName + ".xml", doc);

        error |= suiteResult.isError();
        failure |= suiteResult.isFailure();
        numTests += suiteResult.caseResults.size();
      }
    }
