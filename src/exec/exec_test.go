...
func TestNoBinaryTargetNoOverrideCommand(t *testing.T) {
	...
	err := exec(core.NewDefaultBuildState(), target, ".", nil, false, process.NoSandbox)
	assert.Error(t, err)
...
func TestExec(t *testing.T) {
	...
	err := exec(state, target, "test", []string{"echo", "foo"}, false, process.NoSandbox)
	assert.Nil(t, err)
}
...
func TestCommandExitCode(t *testing.T) {
	...
	exitCode := Exec(state, target.Label, "test", []string{"exit", "5"}, false, process.NoSandbox)
