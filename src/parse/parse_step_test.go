...
func TestAddDepSimple(t *testing.T) {
	...
	state.ActivateTarget(nil, buildLabel("//package1:target1"), core.OriginalTarget, 0)
...
func TestAddDepMultiple(t *testing.T) {
	...
	state := makeState(true, false)
	state.ActivateTarget(nil, buildLabel("//package1:target1"), core.OriginalTarget, 0)
	state.ActivateTarget(nil, buildLabel("//package1:target2"), core.OriginalTarget, 0)
	state.ActivateTarget(nil, buildLabel("//package1:target3"), core.OriginalTarget, 0)
...
func TestAddParseDep(t *testing.T) {
	...
	state := makeState(true, true)
	state.NeedBuild = false
	state.ActivateTarget(nil, buildLabel("//package2:target2"), buildLabel("//package3:all"), 0)
