...
func TestHash(t *testing.T) {
	...
	h := NewPathHasher(wd)
	b1, err1 := h.Hash("src/fs/test_data/test_subfolder1/a.txt", false)
	b2, err2 := h.Hash("src/fs/test_data/test_subfolder1/a.txt", false)
...
func TestMoveHash(t *testing.T) {
	...
	b2, err2 := h.Hash("doesnt_exist.txt", false)
	assert.NoError(t, err1)
	assert.NoError(t, err2)
	assert.EqualValues(t, b1, b2)
}
