...
func TestLimitedPrintfSimple(t *testing.T) {
	...
	assert.Equal(t, "wibble wobble", lprintfPrepare(20, "%s %s", "wibble", "wobble"))
}

func TestLimitedPrintfMore(t *testing.T) {
	assert.Equal(t, "wibble ...", lprintfPrepare(10, "wibble wobble"))
	assert.Equal(t, "wibble ...", lprintfPrepare(10, "%s %s", "wibble", "wobble"))
}
...
func TestLimitedPrintfAnsiNotCountedWhenReducing(t *testing.T) {
	assert.Equal(t, "\x1b[30mwibble ...\x1b[1m", lprintfPrepare(10, "%s %s", "\x1b[30mwibble", "wobble\x1b[1m"))
}
