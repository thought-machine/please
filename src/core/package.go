...
func FindOwningPackage(state *BuildState, file string) BuildLabel {
	// path, not filepath: the result becomes a build label package name, and filepath.Dir on
	// Windows would hand back backslashes, which aren't legal in one.
	f := path.Dir(filepath.ToSlash(file))
	for f != "." {
