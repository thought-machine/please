...
func setJavaDefaults(config *Configuration) {
	// If a java toolchain is configured, use that
	if config.Java.Toolchain != "" {
		// TODO(jpoole): from java9+ jlink should be available in the jdk and we should set it here
		config.Java.JavaTool = config.Java.Toolchain + "|java"
		config.Java.JavacTool = config.Java.Toolchain + "|javac"
	}
	// If javac hasn't been set explicitly, set the javac_worker so the tools use that instead
	if config.Java.JavacTool == "" {
		setDefault(&config.Java.JavacWorker, "javac_worker")
	}

	// If JAVA_HOME has been set, set defaults based on that
	if config.Java.JavaHome != "" {
		setDefault(&config.Java.JlinkTool, filepath.Join(config.Java.JavaHome, "bin/jlink"))
		setDefault(&config.Java.JavaTool, filepath.Join(config.Java.JavaHome, "bin/java"))
		setDefault(&config.Java.JavacTool, filepath.Join(config.Java.JavaHome, "bin/javac"))
	}

	// Otherwise use whatever is on the path
...
func setDefault(conf interface{}, def ...string) {
	if v, ok := conf.(*[]string); ok && len(*v) == 0 {
		*v = def
	} else if v, ok := conf.(*string); ok && *v == "" {
		*v = def[0]
	}
}
