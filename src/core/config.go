...
type Configuration struct {
	...
	Python struct {
		...
		UsePackageRoot      bool     `help:"It makes the python modules to be built at the top level in the .pex file instead of replicating the package path they reside in.\nThis way, imports to workspace python modules are regular imports, as if the modules were locally installed into the python environment.\nThis import behavior is similar to a pip_library module, that was configured with the ModuleDir. However this setting applies for all the python packages, be it an external pip_library or a local python_library." var:"PYTHON_USE_PACKAGE_ROOT"`
