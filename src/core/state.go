...
type isPreloadingKey struct{}

// WithPreloading returns a context noting that we are resolving preloaded subincludes.
// Subsequent package parses using this context must not try to resolve them again to avoid a deadlock.
func WithPreloading(ctx context.Context) context.Context {
	return context.WithValue(ctx, isPreloadingKey{}, struct{}{})
}

// IsPreloading returns true if we are currently resolving preloaded subincludes.
func IsPreloading(ctx context.Context) bool {
	return ctx.Value(isPreloadingKey{}) != nil
}
...
func (state *BuildState) expandOriginalPseudoTarget(label BuildLabel, justTests bool) BuildLabels {
	...
	if label.IsAllTargets() {
		...
		for pkg := range state.Graph.AllPackages() {
			if label.Includes(BuildLabel{PackageName: pkg.Name}) {
				addPackage(pkg)
			}
		}
	}
