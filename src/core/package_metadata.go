...
func (m *packageMetadataImpl) RegisterStatement(stmt BuildStatement, deps BuildLabels) {
	// fmt.Printf("Registering statement %v deps: %v\n", stmt, deps)
	if len(deps) == 0 {
		// Skip adding to the map if the statement doesn't require any subincludes.
		return
	}
