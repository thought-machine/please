...
func TestEmptyProvide(t *testing.T) {
	provideTarget := makeTarget1("//src/core:provider", "PUBLIC")
	requireWhatevs := makeTarget1("//src/core:whatevs", "PUBLIC")
	requireNonsense := makeTarget1("//src/core:nonsense", "PUBLIC")

	provideTarget.Provides = map[string][]BuildLabel{"whatevs": {}}
...
func TestAddDependency(t *testing.T) {
	...
	resolved, unresolved := target2.Dependencies(graphWith(target1, target2))
	assert.Equal(t, []*BuildTarget{target1}, resolved)
	assert.Empty(t, unresolved)
}
...
func TestExternalDependencies(t *testing.T) {
	...
	resolved, unresolved := t2.ExternalDependencies(graph)
	assert.Equal(t, []*BuildTarget{t1}, resolved)
	assert.Empty(t, unresolved)
}
...
// graphWith returns a graph populated with the given targets, for tests that need dependency resolution.
