...
//	log.Warning("Sam: Preparing run...")
	prepareRun()
//	log.Warning("Sam: done.")

//	log.Warning("Sam: Starting run with label %s", label)
...
func run(ctx context.Context, state *core.BuildState, label core.AnnotatedOutputLabel, args []string, fork, quiet, remote, setenv, detach, tmpDir bool, dir, overrideCmd string) ([]byte, []byte, error) {
	...
	out, combined, err := process.New().ExecWithTimeout(ctx, nil, dir, env, time.Duration(math.MaxInt64), false, false, !quiet, process.NoSandbox, args)
