...
func Run(state *core.BuildState, label core.BuildLabel, args []string, env bool, debug bool) {
	run(context.Background(), state, label, args, false, false, env, debug)
}
...
func Parallel(ctx context.Context, state *core.BuildState, labels []core.BuildLabel, args []string, numTasks int, quiet, env bool) int {
	...
	for _, label := range labels {
		...
		g.Go(func() (err error) {
			...
			wg.Add(1)
			pool.Submit(func() {
				if e := run(ctx, state, label, args, true, quiet, env, false); e != nil {
					err = e
				}
				wg.Done()
			})
...
func run(ctx context.Context, state *core.BuildState, label core.BuildLabel, args []string, fork, quiet, setenv bool, debug bool) *exitError {
	...
	if !strings.Contains(splitCmd[0], "/") {
		...
		splitCmd[0] = cmd
		if debug {
			// Obviously this only works if we really are a java -jar
			splitCmd = append(splitCmd[:1], append([]string{"-Xdebug", "-Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=5005"}, splitCmd[1:]...)...)
		}
	}
