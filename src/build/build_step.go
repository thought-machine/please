...
func runBuildCommand(state *core.BuildState, target *core.BuildTarget, command string, inputHash []byte) ([]byte, error) {
	...
	out, combined, err := core.ExecWithTimeoutShell(target, target.TmpDir(), env, target.BuildTimeout, state.Config.Build.Timeout, state.ShowAllOutput, command, state.Config.Build.Sandbox)
	if err != nil {
