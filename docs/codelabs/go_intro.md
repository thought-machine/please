...
The file `.plzconfig` contains the project configuration for Please. Please will have picked up the module's import path
...
Read the [config](/config.html) documentation and [go plugin config](/plugins.html#go.config) for more information on configuration.
