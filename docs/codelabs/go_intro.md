...
We have also provided a `visibilty` list to this rule. This is used to control where this `go_library()` rule can be 
...
This will then download and compile `github.com/go-playground/assert` for us. We use the `package()` builtin function
